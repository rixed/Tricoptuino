...
static void event_program_timer(void)
{
    ...
    do {
        if (unlikely_(e->delay >= MAX_SLEEP)) {
            OCR1A = MAX_SLEEP;
            BIT_SET(TIMSK1, OCIE1A); // enable timer int
            break;
    ...
    while (1) {
        if (! next || delay <= next->delay) {
            ...
            if (! prev) {
                ...
                BIT_CLEAR(TIMSK1, OCIE1A);  // disable timer interrupt
                next_event = e;
                if (next) {
...
{
    BIT_CLEAR(TIMSK1, OCIE1A);  // disable timer interrupt for now
    if (next_event) {
        event_run_next();
        event_program_timer();
    }
...
void event_init(void)
{
    ...
    */
    ...
    BIT_SET(TIFR1, OCF1A);    // clear any pending interrupts; 
}
